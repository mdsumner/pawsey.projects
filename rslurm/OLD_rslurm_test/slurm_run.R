...
library(rslurm, quietly = TRUE, lib.loc = file.path(Sys.getenv("MYSCRATCH"), "Rlib"))
...
saveRDS(.rslurm_id, "slurm_id.rds")
.rslurm_istart <- .rslurm_id * 12 + 1
.rslurm_iend <- min((.rslurm_id + 1) * 12, nrow(.rslurm_params))
