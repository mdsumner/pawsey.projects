...
dates <- seq(as.Date("2002-06-01"), as.Date("2024-02-24"), by = "1 day")
pathname <- function(x) {
...
if (!overwrite && file.exists(out_dsn)) return(NULL)
  ...
  task <- vapour::gdal_raster_dsn(dsnx, out_dsn = out_dsn, options = "-overwrite", target_ext = c(-180, 180, -89.995 , 89.995 ), target_res = 0.01)
 NULL
}
